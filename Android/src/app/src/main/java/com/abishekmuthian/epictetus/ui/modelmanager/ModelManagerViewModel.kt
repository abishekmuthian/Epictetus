...
package com.abishekmuthian.epictetus.ui.modelmanager
...
import com.abishekmuthian.epictetus.data.ModelAllowlist
...
import com.abishekmuthian.epictetus.proto.AccessTokenData
import com.abishekmuthian.epictetus.proto.ImportedModel
import com.abishekmuthian.epictetus.proto.Theme
...
constructor(
  ...
  init {
    Log.d("EpictetusDebug", "ModelManagerViewModel init: customTasks.size=${customTasks.size}")
    customTasks.forEach { task ->
      Log.d("EpictetusDebug", "CustomTask: ${task.task.id} - ${task.task.label}")
    }
  }
  ...
  fun loadModelAllowlist() {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        if (modelAllowlist == null) {
          Log.w(TAG, "Failed to load model allowlist, but proceeding with local models only")
          // Even if allowlist fails, we still want to populate tasks with local models
          val curTasks = customTasks.map { it.task }
          processTasks()
  ...
  private fun createUiState(): ModelManagerUiState {
    Log.d("EpictetusDebug", "createUiState: customTasks.size=${customTasks.size}")
    ...
    for (customTask in customTasks) {
      ...
      Log.d("EpictetusDebug", "Processing CustomTask: ${task.id} - ${task.label} with ${task.models.size} models")
